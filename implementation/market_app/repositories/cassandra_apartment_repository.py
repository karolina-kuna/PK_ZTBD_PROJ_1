...
from market_app.repositories.uuid_util import convert_uuid_into_text, convert_text_into_uuid
...
class CassandraApartmentRepository:
    ...
    def create_table(self):
        query = "CREATE TABLE IF NOT EXISTS apartment (apartment_id uuid PRIMARY KEY, area int, creation_year int, last_renovation_year int, building_type text, heating_type text, is_furnished boolean, rooms_count int)"
        self.session.execute(query)

    def insert(self, apartment: Apartment) -> Apartment:
        query = SimpleStatement(
            "INSERT INTO apartment (apartment_id, area, creation_year, last_renovation_year, building_type, heating_type, is_furnished, rooms_count) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)",
    ...
    def update(self, apartment: Apartment) -> Apartment:
        query = SimpleStatement(
            "UPDATE apartment SET area=%s, creation_year=%s, last_renovation_year=%s, building_type=%s, heating_type=%s, is_furnished=%s, rooms_count=%s WHERE apartment_id=%s",
            consistency_level=ConsistencyLevel.QUORUM
        )
        apartment_id_uuid = convert_text_into_uuid(apartment.apartment_id)
        self.session.execute(query, (
            apartment.area, apartment.creation_year, apartment.last_renovation_year, apartment.building_type,
            apartment.heating_type, apartment.is_furnished, apartment.rooms_count, apartment_id_uuid))
        return apartment
    ...
    def __map_row_to_apartment(self, row) -> Apartment:
        return Apartment(convert_uuid_into_text(row.apartment_id), row.area, row.creation_year,
                         ...
                         row.heating_type, row.is_furnished, row.rooms_count)
