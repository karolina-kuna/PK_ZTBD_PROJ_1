...
class MongoDbOwnerRepository:
    ...
    def create(self, owner: Owner) -> ObjectId:
        return self.collection.insert_one(owner.__dict__).inserted_id

    def update(self, owner: Owner) -> dict:
    ...
    def get_by_id(self, owner_id: str) -> Owner:
        owner_dict = self.collection.find_one({"_id": owner_id})
        return Owner(**owner_dict) if owner_dict is not None else None

    def get_all(self) -> list:
        return [Owner(**owner_dict) for owner_dict in self.collection.find()]

    def find_by_company_name(self, company_name: str) -> list:
        rows = self.collection.find({"company_name": company_name})
        return [Owner(**owner_dict) for owner_dict in rows]
